...
def get_address():
    ...
    addrFile = csv.reader(open(full_path('data.csv'), 'r', encoding='utf-8-sig'))
    rows = list(addrFile)

    if rows and rows[0] and rows[0][0].strip().lower() in {'firstname', '\ufefffirstname'}:
        rows = rows[1:]

    candidates = [r for r in rows if len(r) >= 10 and r[4].strip()]
    row = choice(candidates) if candidates else choice([r for r in rows if len(r) >= 10])
    ...
    return dict(zip(addrParam, values))
